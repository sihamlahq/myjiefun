  ...
  try {
    ...
    const [guests, tables, checkInEvents] = await Promise.all([
      supabase
        .from("guests")
        .select(
          "id, guest_code, name_en, name_zh, phone, table_id, group_id, rsvp_status, attendance_status, expected_count, is_vip, is_walk_in, dietary, relationship, category, notes, checked_in_at, guest_groups(id, name), reception_tables(id, table_number, name, capacity, table_type, status, location)",
        )
