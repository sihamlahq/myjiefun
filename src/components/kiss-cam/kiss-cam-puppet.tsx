...
function ArmChain({
  ...
  return (
    <div
      ...
      style={{ transformOrigin: originPct(shoulder), transform: `rotate(${angles.upper}deg)` }}
    >
      ...
      <div
        ...
        style={{ transformOrigin: originPct(elbow), transform: `rotate(${angles.forearm}deg)` }}
      >
        ...
        <div
          ...
          style={{ transformOrigin: originPct(wrist), transform: `rotate(${angles.hand}deg)` }}
        >
  ...
  return (
    <div className="pointer-events-none absolute inset-0 overflow-visible" data-kiss-arm={which}>
      <div
        ...
        data-kiss-joint="shoulder"
        style={{ left: cqw(shoulder.x), top: cqh(shoulder.y), width: 0, height: 0, transformOrigin: "0 0", transform: `rotate(${angles.upper}deg)` }}
      >
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src={`${base}/${which}-upper-arm.png`} alt="" draggable={false} className="select-none" style={{ ...stageImgStyle, left: cqw(-shoulder.x), top: cqh(-shoulder.y) }} />
        <div
          className={cn("kiss-cam-joint absolute overflow-visible", forearmClass)}
          data-kiss-layer={which === "left" ? "leftForearm" : "rightForearm"}
          data-kiss-joint="elbow"
          style={{ left: cqw(elbowLocal.x), top: cqh(elbowLocal.y), width: 0, height: 0, transformOrigin: "0 0", transform: `rotate(${angles.forearm}deg)` }}
        >
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={`${base}/${which}-forearm.png`} alt="" draggable={false} className="select-none" style={{ ...stageImgStyle, left: cqw(-elbow.x), top: cqh(-elbow.y) }} />
          <div
            ...
            style={{ left: cqw(wristLocal.x), top: cqh(wristLocal.y), width: 0, height: 0, transformOrigin: "0 0", transform: `rotate(${angles.hand}deg)` }}
          >
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={`${base}/${which}-hand.png`} alt="" draggable={false} className="select-none" style={{ ...stageImgStyle, left: cqw(-wrist.x), top: cqh(-wrist.y) }} />
            {balloon ? (
              <div
                className="kiss-cam-held-balloon pointer-events-none absolute"
                style={{ left: cqw(rig.handRest[which].x - wrist.x), top: cqh(rig.handRest[which].y - wrist.y), width: cqw(Math.round(STAGE_W * 0.14)), height: cqh(Math.round(STAGE_H * 0.17)), transform: "translate(-50%, -100%)", transformOrigin: "50% 100%" }}
              >
                <svg viewBox="0 0 60 90" className="h-full w-full overflow-visible" aria-hidden>
                  <line x1="30" y1="38" x2="30" y2="88" stroke="#9a8b82" strokeWidth="1.4" strokeLinecap="round" />
                  <path d="M30 32 C50 12 56 -8 30 2 C4 -8 10 12 30 32Z" fill={balloon.color} stroke="#5a4a42" strokeWidth="1.5" />
                  <ellipse cx="22" cy="12" rx="4" ry="6" fill="rgba(255,255,255,0.4)" />
                </svg>
              </div>
            ) : null}
...
function buildDebug(side: "groom" | "bride", rig: CharacterRigJoints, holdTarget: { x: number; y: number }, kissTarget: { x: number; y: number }, showHold: boolean) {
  ...
  if (rig.veilPivot) {
    pivots.push({ id: "veil", label: "veil", x: pct(rig.veilPivot.x, "x"), y: pct(rig.veilPivot.y, "y"), color: "#93c5fd" });
  }
...
export function GroomFigure({ phase, className, rigDebug = false }: PuppetProps) {
  ...
  return (
    <div
      className={cn("kiss-cam-figure pointer-events-none absolute bottom-0 h-full origin-bottom", className)}
      style={{
        height: "100%",
        // The groom root contains only absolutely-positioned artwork, so width:auto
        // collapses. Give it the exact 720:1380 canvas width from the stage height.
        width: "calc(100cqh * 0.5217391304)",
    ...
    >
      <div
        className={cn("relative h-full w-full overflow-visible", FIGURE_HEIGHT_CLASS)}
        style={{ width: "100%", height: "100%", aspectRatio: `${STAGE_W} / ${STAGE_H}`, containerType: "size" }}
      >
    ...
    >
      ...
      >
        <div className="absolute inset-0 overflow-visible drop-shadow-[0_14px_28px_rgba(60,50,40,.18)]">
          ...
          <div className="kiss-cam-veil absolute inset-0 overflow-visible" style={{ transformOrigin: originPct(veilPivot), transform: `rotate(${resolved.veilRot}deg)` }}>
          ...
          <div className="kiss-cam-dress absolute inset-0" style={{ transformOrigin: originPct(rig.hipPivot), transform: `rotate(${resolved.bodyRot * 0.25}deg)` }}>
            <LayerImg src={`${base}/skirt.png`} />
          </div>
          <div className="kiss-cam-body absolute inset-0" style={{ transformOrigin: originPct(rig.bodyPivot), transform: `rotate(${resolved.bodyRot}deg)` }}>
            ...
            <div className="kiss-cam-head absolute inset-0 z-[2]" style={{ transformOrigin: originPct(rig.headPivot), transform: `rotate(${resolved.headRot}deg)` }}>
              <LayerImg src={`${base}/head.png`} className="kiss-cam-face" />
              <LayerImg src={`${base}/hair.png`} />
